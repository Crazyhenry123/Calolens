...
namespace Microsoft.MixedReality.OpenXR
{
            ...
            if (anchorStoreLoadTask == null)
            {
                ...
                if (feature != null && feature.enabled)
                {
                    anchorStoreLoadTask = Task.Run(() =>
                    {
                        ...
                        return new OpenXRAnchorStore(nativeAnchorStoreLoaded);
                    });
                }
                else
                {
    ...
    internal class OpenXRAnchorStore
    {
        ...
        private bool m_nativeAnchorStoreLoaded = false;
        private readonly object m_persistedAnchorNamesCacheLock = new object();

        internal OpenXRAnchorStore(bool nativeAnchorStoreLoaded)
        {
            m_nativeAnchorStoreLoaded = nativeAnchorStoreLoaded;
        }
        ...
        {
            Guid persistedAnchor = Guid.Empty;
            if (m_nativeAnchorStoreLoaded)
            {
                persistedAnchor = NativeLib.LoadPersistedAnchor(nativeLibToken, name);
            }
